...
{
public:
    explicit MailboxCore(const char *name);
...
template<typename T>
Mailbox<T>* MailboxManager::find(const char *name)
{
...
template <class T>
Mailbox<T>::Mailbox(const char *name, uint32_t len) : MailboxCore(name)
{
    _handler = xQueueCreate(len, sizeof(T));
    MailboxManager::add(this);
}
