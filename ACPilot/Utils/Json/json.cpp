...
namespace Utils
{
    class FreeJsonTreeVisit : public Common::TreeVisit
    {
    public:
    ...
    class SerializeJsonVisit : public Common::TreeVisit
    {
    public:
...
AC_RET FreeJsonTreeVisit::operator()(Common::Tree *node, Common::TREE_VISIT_ORDER order)
{
    JsonTree *tree = static_cast<JsonTree *>(node);
    JsonTree *child = tree->getFirstChild();
    JsonTree *next = nullptr;

    if (Common::TREE_POST_ORDER == order)
    {
        while (nullptr != child)
        {
            next = child->getNeighbor();
            Json::_pool->free((uint8_t *) child);
            child = next;
        }
    }
    return AC_OK;
...
AC_RET SerializeJsonVisit::operator()(Common::Tree *node, Common::TREE_VISIT_ORDER order)
{
    JsonTree *tree = static_cast<JsonTree *>(node);
    int len = 0;
...
bool JsonTree::jsonArrayNodeIsUsed(JsonTree *node)
{
    if (nullptr == node)
    {
        return false;
...
void JsonTree::jsonArrayNodeSetIndex(JsonTree *node, uint16_t index)
{
    snprintf(node->getKey(), PARAM_NAME_LEN, "%u", index);
}

uint16_t JsonTree::jsonArrayNodeGetIndex(JsonTree *node)
{
...
AC_RET JsonTree::traverse(Common::TreeVisit &visit)
{
...
JsonTree *Json::alloc()
{
    if (nullptr == _pool)
    {
        _pool = MemoryPool::find(JSON_MEMORY_POOL_NAME);
        if (nullptr == _pool)
        {
