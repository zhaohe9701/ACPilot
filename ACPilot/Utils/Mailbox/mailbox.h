...
/* ---------------------- MailboxCore ------------------------- */
    ...
    public:
        ...
        static Common::List<MailboxCore *> *getList();

    protected:
        static Common::List<MailboxCore *> _list;
...
/* ------------------------- Mailbox ---------------------------- */
    ...
    private:
        Osal::QueueHandle _handler = nullptr;
    ...
    {
        for (Common::ListNode<MailboxCore *> *it = _list.begin(); it != _list.end(); it = it->getNext())
        {
            if ((**it)->match(name))
            {
