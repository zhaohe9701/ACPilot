...
Icm20602::Icm20602(DeviceInterface *interface)
{
    _interface = interface;
    strncpy(_name, "ICM20602", PARAM_NAME_LEN);
    _READ = READ;
    _WRITE = WRITE;
}
void Icm20602::init()
...
AC_RET Icm20602::readTmp(TmpData &data)
{
    _tmp_mutex.lock();
    data = _tmp_data;
    _tmp_mutex.unlock();
...
AC_RET Icm20602::readGyro(GyroData &data)
{
    _gyro_mutex.lock();
    data = _gyro_data;
    _gyro_mutex.unlock();
...
AC_RET Icm20602::readAcc(AccData &data)
{
    _acc_mutex.lock();
    data = _acc_data;
    _acc_mutex.unlock();
