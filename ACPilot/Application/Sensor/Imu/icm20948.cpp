...
Icm20948::Icm20948(DeviceInterface *interface)
{
...
AC_RET Icm20948::updateTmp()
{
    ...
    _tmp_mutex.lock();
    _tmp_data = 25.0f + (float)raw / 326.8f;
    _tmp_mutex.unlock();
    return AC_OK;
}

AC_RET Icm20948::updateGyro()
{
    uint8_t buf[6];
...
AC_RET Icm20948::readTmp(TmpData &data)
{
    _tmp_mutex.lock();
    data = _tmp_data;
    _tmp_mutex.unlock();
...
AC_RET Icm20948::readGyro(GyroData &data)
{
    _gyro_mutex.lock();
    data = _gyro_data;
    _gyro_mutex.unlock();
...
AC_RET Icm20948::readAcc(AccData &data)
{
    _acc_mutex.lock();
    data = _acc_data;
    _acc_mutex.unlock();
